        ...
        <div className="mb-4">
          {categories.map(cat => (
            <button
              ...
              className={`btn btn-sm m-1 ${selected === cat ? 'btn-primary' : 'btn-outline-primary'}`}
            >
              {cat}
            </button>
          ))}
        ...
        <div className="row g-4">
          {filteredProducts.map(product => (
            <div className="col-md-3 col-sm-6" key={product.id}>
              <div className="card product-card h-100 shadow-sm border-0 position-relative">
                {product.tag && (
                  <span className="badge bg-success position-absolute top-0 start-0 m-2">{product.tag}</span>
                )}
                <img
                  src={product.image}
                  alt={product.name}
                ...
                <div className="card-body">
                  ...
                  <p className="text-muted small">{product.desc}</p>
                  <p className="fw-bold text-primary">{product.price}</p>
                  <p className={`small ${product.stock ? 'text-success' : 'text-danger'}`}>
